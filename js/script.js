	...
	function setTabsAction(e) {
		...
		if (tabTitle) {
			...
			if (!tabsBlock.querySelector('._slide')) {
				...
				if (isSpoller) {
					// Аккордеон: клик на активный закрывает его, иначе открывает
					...
					// Обычные табы: переключение только при клике на неактивный
					if (!tabTitle.classList.contains('_tab-active')) {
						tabsBlock.querySelectorAll('[data-tabs-title]._tab-active').forEach(item => item.classList.remove('_tab-active'));
						tabTitle.classList.add('_tab-active');
						setTabsStatus(tabsBlock);
...
const animationConfig = [
	{ elements: '.process', delay: 0.1 },
	{ elements: '.contact__wrapper', delay: 0.1 },
	{ elements: '.contact__desc', delay: 0.4, direction: 'left-right' },
	{ elements: '.contact__form', delay: 0.7, direction: 'left-right' },
];
